...
### Using Bayesian hierarchical modeling to determine if qSIP activity correlates with transcription? 
Using R packages cmdstanr and brms, I modeled qSIP activity (Excess Atom Fraction, EAF) using a zero inflated beta model with fixed effects for expression (transcription), time since rewet (48h vs 168h), and moisture (50% vs 100%), and random intercepts and expression slopes by MAG. 
